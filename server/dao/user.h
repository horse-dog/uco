 ...
 *
 * 模型约定:
 *   - dao 出入参一律 proto 化 (dao/user.proto, package webserver.dao):
...
#include "core/uco.h"
#include "server/dao/user.pb.h" // user 领域模型与出入参 (package webserver.dao)
...
class IUserDao
{
  public:
     ...
     */
    virtual uco::task<int> InsertUser(const dao::InsertUserReq &req,
                                      dao::InsertUserRsp &rsp) = 0;
     ...
     *         -1/-2 系统错误 (不可重试/可重试).
     */
    virtual uco::task<int> QueryByUsername(const dao::QueryByUsernameReq &req,
                                           dao::QueryByUsernameRsp &rsp) = 0;
     ...
     *         -1/-2 系统错误 (不可重试/可重试).
     */
    virtual uco::task<int> QueryByVid(const dao::QueryByVidReq &req,
                                      dao::QueryByVidRsp &rsp) = 0;

    /**
     * @brief 修改密码.
     * @return 0 成功; 1 无此用户; -1/-2 系统错误 (不可重试/可重试).
     */
    virtual uco::task<int> UpdatePassword(const dao::UpdatePasswordReq &req,
                                          dao::UpdatePasswordRsp &rsp) = 0;
};
