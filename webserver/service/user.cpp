...
/// 用户名字符白名单 [A-Za-z0-9_-] (见 user.proto RegisterReq 契约):
...
/// 密码含危险字符: 控制字符 / 首尾空白
/// (黑名单式, 不做白名单 — 见 user.proto RegisterReq 契约).
...
uco::task<int> UserService::Register(const user::RegisterReq &req,
                                     user::RegisterRsp &rsp)
{
    // 1. 参数校验 (规则契约见 user.proto RegisterReq 注释).
...
uco::task<int> UserService::Login(const user::LoginReq &req,
                                  user::LoginRsp &rsp)
{
    // 1. 取认证凭据 (dao 返回码透传: 1 无此用户, -1/-2 系统).
    user::QueryByUsernameReq dreq;
    dreq.set_username(req.username());
    user::QueryByUsernameRsp drsp;
