...
#include "dao/user.h" // dao 层接口 (webserver::dao::IUserDao)
...
class IUserService
{
  public:
     ...
     *        规则契约见 user.proto RegisterReq 注释 (前后端单一文档源).
     * @return 0 成功 (rsp 已填充);
     *         1 用户名已被占用; 2 参数非法 (长度);
     *         3 密码过于简单 (黑名单/含用户名/危险字符);
     *         -1/-2 系统错误 (不可重试/可重试).
     */
    virtual uco::task<int> Register(const user::RegisterReq &req,
                                    user::RegisterRsp &rsp) = 0;
     ...
     *         -1/-2 系统错误 (不可重试/可重试).
     */
    virtual uco::task<int> Login(const user::LoginReq &req,
                                 user::LoginRsp &rsp) = 0;
};
...
class UserService final : public IUserService
{
  public:
     ...
     *         -1/-2 系统错误 (不可重试/可重试).
     */
    uco::task<int> Register(const user::RegisterReq &req,
                            user::RegisterRsp &rsp) override;
     ...
     *         -1/-2 系统错误 (不可重试/可重试).
     */
    uco::task<int> Login(const user::LoginReq &req,
                         user::LoginRsp &rsp) override;
